...
author: Madison & Magdiel Miranda
---
...
```{r}
length(unique(pharmacoData$concentration))
```
...
```{r}
 pharmacoData %>%
    ...
    geom_histogram(fill = "gray", color = "black", binwidth = 30) +
    facet_wrap(~ study) +
...
range(pharmacoData$viability)
    sum(pharmacoData$viability < 0) #23 lower than 0.
    sum(pharmacoData$viability > 100) #15778 over 100. 
    sum(pharmacoData$viability > 0 & pharmacoData$viability < 100) #27611
...
```{r AUC-all-drug-scaled, fig.width = 10, fig.height = 9}
summarizedData %>%
    ggplot(aes(x = -log10(auc_GDSC / 10^6),
               y = -log10(auc_CCLE / 10^6))) +
    geom_point(cex = 1/2) + 
    facet_wrap(~ drug) +
    xlab("-log10(auc_GDSC/10^6)") +
    ylab("-log10(auc_CCLE/10^6)") +
...
```{r}
drugCorrs_auc <- gather(drugCorrs_auc, measure, correlation, -drug)
...
3. Based on the scatterplot and correlation values, would you say that they tend to agree? Why or why not?
    ...
    Some of them more than others but the correlation stills low.  
...
5. Which drug do you think shows the most consistency between the studies? How about the least? 
    ...
    most consistency: Erlotinib 
    less consistency 17-AAG
